...
def minmax(subs_arr, sum1, sum2, is_max, memoization):
    if len(subs_arr) ==0:
        calculated_score =  calc_score(sum1, sum2)
        return calculated_score, None
    #if get_hash(subs_arr) in memoization:
    #    return memoization[get_hash(subs_arr)]
    score = 0
    lr_move = None
    ...
    if is_max:
        max_score = -math.inf 
        for move in moves:
                score, _ = minmax(move[1], sum1+move[0], sum2, not is_max, memoization)
                if max_score < score:
                    max_score = score
                    lr_move = move[0] 

                    #memoization[get_hash(subs_arr)] = max_score, lr_move
        return max_score, lr_move
    else:
        min_score = math.inf
        for move in moves:
            score, _ = minmax(move[1], sum1, sum2+move[0], not is_max, memoization)
            if min_score > score:
                min_score = score
                lr_move = move[0]
        return min_score, lr_move
                #memoization[get_hash(subs_arr)] = min_score, lr_move
    #return memoization[get_hash(subs_arr)]
